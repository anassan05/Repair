...
const BookingModal = ({ isOpen, onClose, currentUser }: BookingModalProps) => {
  ...
  const handleSubmit = () => {
    ...
    // Create new booking
    const newBooking = {
      id: `REP-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,
      device: `${deviceType === "laptop" ? "Laptop" : "Desktop PC"} - ${selectedBrand}`,
    ...
    const existingBookings = JSON.parse(localStorage.getItem('repairBookings') || '[]');
    localStorage.setItem('repairBookings', JSON.stringify([newBooking, ...existingBookings]));
