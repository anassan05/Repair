...
export default function Dashboard() {
  ...
  const handleCloseBooking = async () => {
    if (!closeOtp || closeOtp.length !== 6) {
      alert('Please enter the 6-digit OTP from the customer');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/worker/close-booking', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(data.message || 'Error closing booking');
      }
    } catch (error) {
      ...
      <Dialog open={closeBookingDialogOpen} onOpenChange={setCloseBookingDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto mx-4 sm:mx-0">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter customer OTP to verify and complete the service
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {selectedBooking && (
            ...
            <div className="space-y-2">
              <Label htmlFor="closeOtp">Customer OTP *</Label>
              <Input
                id="closeOtp"
                type="text"
                placeholder="Enter 6-digit OTP"
                ...
                maxLength={6}
                className="text-center text-xl sm:text-2xl font-mono tracking-widest"
              />
              <p className="text-sm text-muted-foreground">
                Ask the customer for their booking OTP to verify service completion
              </p>
            </div>
