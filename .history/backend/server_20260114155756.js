...
app.post('/api/worker/close-booking', upload.single('completionImage'), (req, res) => {
  const { bookingId, otp, usedNewComponents, componentDetails, warrantyMonths } = req.body;
  ...
  // Verify booking exists and OTP matches
  const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(bookingId);
