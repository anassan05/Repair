...
import { Users, Plus, Edit, Trash2, UserCheck, UserX, Save, X, ArrowLeft, LogOut } from "lucide-react";
...
const WorkerManagement = () => {
  ...
  const handleAddWorker = async () => {
    if (!formData.name || !formData.email || !formData.password || !formData.phone) {
      toast({
        title: "Error",
        description: "Please fill in all required fields (name, email, phone, password)",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: "Success",
          description: `Worker ${formData.name} added successfully. They can login with email: ${formData.email}`,
        });
        loadWorkers();
        ...
        toast({
          title: "Error",
          description: data.message || "Failed to add worker",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleEditWorker = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/admin/workers/${selectedWorker.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
      ...
      if (data.success) {
        ...
        toast({
          title: "Error",
          description: data.message || "Failed to update worker",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleDeleteWorker = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({
          title: "Error",
          description: data.message || "Failed to delete worker",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const toggleWorkerStatus = async (worker: Worker) => {
    try {
      const response = await fetch(`http://localhost:3000/api/admin/workers/${worker.id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        toast({
          title: "Error",
          description: data.message || "Failed to update worker status",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new technician to your team. They will use email and password to login.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                id="email"
                type="email"
                placeholder="worker@techfix.com"
            ...
            <div>
              <Label htmlFor="password">Password * (Used for login)</Label>
              <Input
                id="password"
                type="password"
